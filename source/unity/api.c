...
static void drawpage(fz_context *ctx, fz_document *doc, int pagenum, PageList *pagelist)
{
    ...
    else
    {
        ...
        fz_try(ctx)
        {
            ...
            Page *page = &pagelist->pages[pagenum - 1];
            page->height = 100;
            page->width = 100;
    ...
    PageList *pagelist)
{
    ...
    pagelist->count = pagecount;
    pagelist->pages = (Page *)malloc(pagecount * sizeof(Page));
    ...
    while ((range = fz_parse_page_range(ctx, range, &spage, &epage, pagecount)))
    {
        if (spage < epage)
            ...
            {
                fz_try(ctx)
                    drawpage(ctx, doc, page, pagelist);
                fz_catch(ctx)
                {
                    if (ignore_errors)
...
int DrawPdfPages(char *filename, PageList *pagelist)
{
    ...
    //         case 'R':
    //             rotation = fz_atof(fz_optarg);
    //             break;
    //         case 'r':
    //             resolution = fz_atof(fz_optarg);
    //             res_specified = 1;
    //             break;
    //         case 'w':
    //             width = fz_atof(fz_optarg);
    //             break;
    //         case 'h':
    //             height = fz_atof(fz_optarg);
    //             break;
    //         case 'f':
    //             fit = 1;
    //             break;
    //         case 'B':
    //             band_height = atoi(fz_optarg);
    //             break;

    //         case 'c':
    //             out_cs = parse_colorspace(fz_optarg);
    //             break;
    //         case 'e':
    //             proof_filename = fz_optarg;
    //             break;
    //         case 'G':
    //             gamma_value = fz_atof(fz_optarg);
    //             break;
    //         case 'I':
    //             invert++;
    //             break;
    ...
    //         case 's':
    //             if (strchr(fz_optarg, 't'))
    //                 ++showtime;
    //             if (strchr(fz_optarg, 'm'))
    //                 ++showmemory;
    //             if (strchr(fz_optarg, 'f'))
    //                 ++showfeatures;
    //             if (strchr(fz_optarg, '5'))
    //                 ++showmd5;
    //             break;
    ...
    //             break;
    //         case 'l':
    //             min_line_width = fz_atof(fz_optarg);
    //             break;
    //         case 'i':
    //             ignore_errors = 1;
    //             break;
    ...
    //         case 'm':
    //             if (fz_optarg[0] == 's')
    //                 trace_info.mem_limit = fz_atoi64(&fz_optarg[1]);
    //             else if (fz_optarg[0] == 'a')
    //                 trace_info.alloc_limit = fz_atoi64(&fz_optarg[1]);
    //             else
    //                 trace_info.mem_limit = fz_atoi64(fz_optarg);
    //             break;
    //         case 'L':
    //             lowmemory = 1;
    //             break;
    ...
    //         case 'v':
    //             fprintf(stderr, "mudraw version %s\n", FZ_VERSION);
    //             return 1;
    //         }
    //     }
    ...
    fz_try(ctx)
    {
        ...
        fz_try(ctx)
        {
            ...
            fz_try(ctx)
            {
                ...
                drawrange(ctx, doc, "1-N", pagelist);
